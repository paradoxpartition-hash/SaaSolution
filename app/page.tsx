        ...
        >
          <div className="relative h-full bg-gradient-to-b from-[#ffffff] to-[#f2f4f8] p-4 md:p-6">
            ...
            <div className="relative h-full rounded-2xl border border-[#d8dbe2]/80 bg-white/90 p-4 backdrop-blur-sm">
              <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold md:text-base">SaaSolution SL Project Showcase</h3><span className="rounded-full border border-[#b89c6a]/30 bg-[#fffaf0] px-3 py-1 text-xs text-[#7b6438]">Live Product Ecosystem</span></div>
              <div className="relative h-[58%] rounded-xl border border-[#d8dbe2] bg-gradient-to-br from-[#f6f8fc] to-[#eef2f8] p-4 shadow-sm"><div className="mb-2 text-xs font-medium uppercase tracking-[0.2em] text-[#7e8595]">Featured Platform</div><div className="mb-2 text-xl font-semibold">Viyra</div><div className="grid grid-cols-3 gap-2 text-xs"><div className="rounded-lg bg-white p-2 shadow-sm">Property Intake AI</div><div className="rounded-lg bg-white p-2 shadow-sm">Automation Workflows</div><div className="rounded-lg bg-white p-2 shadow-sm">Owner Dashboard</div></div></div>
              <div className="relative mt-4 h-[34%] overflow-hidden"><div className="project-marquee absolute left-0 top-1/2 flex -translate-y-1/2 gap-3">{[...projects, ...projects].map((project, idx) => <div key={`${project.name}-${idx}`} className={`w-[190px] shrink-0 rounded-xl border border-[#d8dbe2] bg-gradient-to-br ${project.tone} p-3 shadow-sm`} style={{ transform: `rotate(${idx % 2 === 0 ? -2 : 2}deg)` }}><div className="mb-5 h-10 rounded-md bg-white/85" /><div className="text-xs font-semibold">{project.name}</div><div className="text-[11px] text-[#5b6478]">{project.tag}</div></div>)}</div></div>
            </div>
          </div>
        </ContainerScroll>
      </section>
